...
url = 'http://127.0.0.1:3000/Pertemuan 1/Index.html'
...
if response.status_code == 200:
    ...
    pengalaman = soup.find('article', id='pengalaman')
    if pengalaman:
        paragraphs = pengalaman.find_all('li')
        for idx, p in enumerate(paragraphs, 1):
            print(f'List {idx} dalam pengalaman:', p.get_text())
    else: 
        print("Tidak ada")

    keterampilan = soup.find('article', id='keterampilan')
    if keterampilan:
        paragraphs = keterampilan.find_all('tr')
        for idx, p in enumerate(paragraphs, 1):
            print(f'Tabel {idx} dalam keterampilan:', p.get_text())
    else: 
        print("Tidak ada")
